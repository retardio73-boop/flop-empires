...
function renderPrelaunch(){const el=$('#prelaunchPanel'),s=state?.season,v=state?.viewer||{},emp=myEmpire(),claimed=state.world.empires.filter(e=>e.claimed).length;if(!el)return;if(s.status!=='REGISTRATION'){el.hidden=true;el.innerHTML='';const lock=$('#gameplayLock');if(lock)lock.textContent='';return}el.hidden=false;const player=emp?('Registered · '+esc(emp.name)):(v.authenticated?'Identity ready · choose an empire':'Observer mode');const action=emp?'<button class="prelaunch-cta" data-prelaunch="player">Open player dashboard</button>':'<button class="prelaunch-cta" data-prelaunch="empires">Choose an empire</button>';el.innerHTML='<div class="prelaunch-copy"><span class="eyebrow">PRE-LAUNCH</span><h2>Season 0 is not live yet</h2><p>Gameplay is locked while registration is open. Explore the world, authenticate, and claim an empire without starting combat or economy actions.</p></div><div class="prelaunch-actions"><div><strong>'+player+'</strong><small>'+claimed+'/16 empire slots claimed · starts '+time(s.season_start)+'</small></div>'+action+'<button class="ghost-btn" data-prelaunch="map">Explore map</button></div>';$$('[data-prelaunch]',el).forEach(b=>b.onclick=()=>switchTab(b.dataset.prelaunch));const lock=$('#gameplayLock');if(lock)lock.textContent='● GAMEPLAY LOCKED · REGISTRATION ONLY'}
